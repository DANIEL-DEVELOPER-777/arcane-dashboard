...
export function EquityChart({ data, onPeriodChange, isLoading }: EquityChartProps) {
  ...
  if (isLoading) {
    return (
      <div className="w-full h-[350px] bg-white/5 rounded-3xl animate-pulse flex items-center justify-center">
  ...
  if (!data || data.length === 0) {
    return (
      <div className="w-full h-[350px] bg-white/5 rounded-3xl flex items-center justify-center border border-white/5">
  ...
  return (
    <div className="glass-panel rounded-3xl p-6 md:p-8 relative overflow-hidden group">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 gap-4">
        <div>
          <h3 className="text-muted-foreground text-sm font-medium uppercase tracking-wider mb-1">Equity Curve</h3>
                ...
                if (active && payload && payload.length) {
                  return (
                    <div className="bg-zinc-900/90 backdrop-blur-xl border border-white/10 p-3 rounded-xl shadow-2xl">
                      <p className="text-zinc-400 text-xs mb-1">{format(new Date(label), "MMM d, yyyy HH:mm")}</p>
                      <p className="text-white font-bold text-lg">
                        {formatCurrency(payload[0].value as number)}
                      </p>
                      <p className="text-emerald-400 text-xs mt-1 font-medium">Equity</p>
                    </div>
                  );
                }
                return null;
